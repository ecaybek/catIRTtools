...
ui <- navbarPage(title="catIRT Tools", theme = shinytheme("cerulean"),
   ...
   sidebarLayout(
     sidebarPanel(
       ...
       shinyjs::hidden(div(id = "itemupload",
           fileInput("parameter.file","File for Item Parameters"),
           fluidRow(column(6, radioButtons("seperator.par", "Seperator for File", c(Semicolon = ";", Comma = ","))),
       ...
       shinyjs::hidden(div(id = "responseupload", 
           fileInput("response.file", "File for Responses"),
           fluidRow(column(6, radioButtons("seperator.res", "Seperator for File", c(Semicolon = ";", Comma = ","))),
     ...
     # Show results
      mainPanel(
          ...
          shinyjs::hidden(div(id="results.allm", 
                              print(p(strong("You can download data & results below"))),
                              fluidRow(column(4, downloadButton("download.items","Download Item Parameters")),
                              ...
                              br(),
                              fluidRow(column(4, downloadButton("download.items.comma","Download Item Parameters (with Comma)")),
   ...
   tabPanel("IRT Analysis",
            ...
            sidebarLayout(
              ...
              mainPanel(
                shinyjs::hidden(div(id="results.mirt.download", 
                                    print(p(strong("You can download results below"))),
                                    fluidRow(column(4, downloadButton("download.mirt.itempar","Download Item Parameters")),
                                             column(4, downloadButton("download.mirt.theta", "Download Theta Estimates"))),
                                             #column(4, downloadButton("download.results", "Download Simulation Results"))),
                                    br(),
                                    fluidRow(column(4, downloadButton("download.mirt.itempar.comma","Download Item Parameters (with Comma)")),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$irtmodel, { if(input$irtmodel != "GRM" & input$irtmodel != "GPCM" ){
  ...
  for(i in 1:nrow(responses.GRM)){ # Run the loop for everyone
    while(serror > input$stderr | length(aditem) < input$minitem) { # Stopping rule
      ...
      if(length(aditem) == nrow(items.GRM)){break}
    }
  ...
  observeEvent(input$csvdecimal, { if(input$csvdecimal == TRUE){
    theta.n.items <- format(theta.n.items, decimal.mark = ",")
  } })
  
  output$download.items <- downloadHandler(
    filename = function() {"itempar.csv"} ,
    content = function(file) {
    write.table(items.GRM, file, sep = ";",
                  row.names = FALSE)
    }
  )
  output$download.items.comma <- downloadHandler(
    filename = function() {"itempar.csv"} ,
    content = function(file) {
      write.table(format(items.GRM, decimal.mark = ","), file, sep = ";",
                  row.names = FALSE)
    }
  )
  output$download.responses <- downloadHandler(
    ...
    content = function(file) {
      write.table(responses.GRM, file, sep = ";",
                  row.names = FALSE)
    }
  )
  output$download.responses.comma <- downloadHandler(
    filename = function() {"responses.csv"} ,
    content = function(file) {
      write.table(format(responses.GRM, decimal.mark = ","), file, sep = ";",
                  row.names = FALSE)
    }
  )
  output$download.results <- downloadHandler(
    filename = function() {"itempar.csv"} ,
    content = function(file) {
      write.table(theta.n.items, file, sep = ";",
                  row.names = FALSE)
    }
  )
  output$download.results.comma <- downloadHandler(
    filename = function() {"itempar.csv"} ,
    content = function(file) {
      write.table(format(theta.n.items, decimal.mark=","), file, sep = ";",
                  row.names = FALSE)
    }
  )
  ...
  observeEvent(input$mirt.calibrate,{
    ...
    output$download.mirt.itempar <- downloadHandler(
      filename = function() {"mirt.itempar.csv"} ,
      content = function(file) {
        write.table(mirt.item.par.f$items, file, sep = ";",
                    row.names = FALSE)
      })
    output$download.mirt.itempar.comma <- downloadHandler(
      filename = function() {"mirt.itempar.comma.csv"} ,
      content = function(file) {
        write.table(format(mirt.item.par.f$items, decimal.mark = ","), file, sep = ";",
                    row.names = FALSE)
      })
    output$download.mirt.theta <- downloadHandler(
      filename = function() {"mirt.theta.csv"} ,
      content = function(file) {
        write.table(mirt.theta, file, sep = ";",
                    row.names = FALSE)
      })
    output$download.mirt.theta.comma <- downloadHandler(
      filename = function() {"mirt.theta.comma.csv"} ,
      content = function(file) {
        write.table(format(mirt.theta, decimal.mark = ","), file, sep = ";",
                    row.names = FALSE)
      })
