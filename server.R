...
server <- function(input, output, session) {
  ...
  observeEvent(input$submit, {
    ...
    if (is.null(input$file_par)) {
      ...
      ItemPool <- read.csv(input$file_par$datapath,
    ...
    if (is.null(input$file_res)) {
      ...
      Responses <- read.csv(input$file_res$datapath,
    ...
    Correlation <- round(cor(ThetaNItems[, 1], ThetaNItems[, 2]), 2)
    MeanSE <- round(apply(ThetaNItems["se"], 2, mean), 2)
    MeanK <- round(mean(rowSums(!is.na(AdministeredItems))), 2)
